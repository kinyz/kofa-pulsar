...
import (
	...
	"kofa-pulsar/core/reflect"
	"kofa-pulsar/core/service"
	"log"
	"os"
	"os/signal"
	"syscall"
...
func (s *IServer) OpenListen() {

	if err := s.message.Listen(DiscoveryTopic+s.Info().GetId(), DiscoveryTopic, s.Info().GetId(),s.info.Name); err != nil {
		panic(err)
	}
